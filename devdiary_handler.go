	...
	*/
	...
	if command == "devdiary" {
		...
		if year == "2017" || year == "17" {
			...
			if month == "august" || month == "8" || month == "aug" {
				diarylink := "DevDiary for August 2017:\n"
				diarylink = diarylink + "https://www.youtube.com/watch?v=ageLN5r6Twc"
				s.ChannelMessageSend(m.ChannelID, diarylink)
				return
			}
			if month == "October" || month == "10" || month == "oct" {
				diarylink := "DevDiary for October 2017:\n"
				diarylink = diarylink + "https://www.youtube.com/watch?v=hxawXXUqZkw"
				s.ChannelMessageSend(m.ChannelID, diarylink)
				return
			}
